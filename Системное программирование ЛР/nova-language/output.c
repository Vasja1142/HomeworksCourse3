...
int get_int() {
    char buf[100]; printf("> "); fflush(stdout);
    if (!fgets(buf, 100, stdin)) exit(0);
    return atoi(buf);
}
char* get_str() {
    char buf[256]; printf("> "); fflush(stdout);
    if (!fgets(buf, 256, stdin)) exit(0);
    buf[strcspn(buf, "\n")] = 0;
    return strdup(buf);
}
...
int main() {
	...
	print_any("----------------------------------------------");
	print_any("[MATH] Расчет эффективности (99.0 * 0.55):");
	double base_efficiency = 99.0;
